...
local function ValidateKey(key)
    ...
    local url = string.format(
        "%s/%s?key=%s&hwid=%s&robloxId=%s&placeId=%s",
        API_CONFIG.base_url,
    ...
    if decoded.status == "ok" then
        ...
        return true, decoded.code or "OK"
    end

    return false, decoded.code or "INVALID"
end
...
local Quest, Event, Trade, Enchant, Discord, Config, Setting, Misc
...
local Sec=Auto:Section({Title="Main Feature"})
local AFR=false
local RS=game:GetService("ReplicatedStorage")
...
local favRarityDropdown = Reg("favRarity",Auto:Dropdown({
 Title="Rarity",
 Values=rar,
 Value={"Legendary","Mythic","SECRET"},
 Multi=true,
 Callback=function(v)tiers=v end
}))

local favMutationDropdown = Reg("favMutation",Auto:Dropdown({
 Title="Mutation",
 Values=allM,
 Value={},
 Multi=true,
 Callback=function(v)muts=v end
}))

local autoFavoriteToggle = Reg("autoFavorite",Auto:Toggle({
...
local function refreshPlayerList()
 ...
 if currentCount~=lastPlayerCount then
  DD:SetValues(currentPlayers)
  lastPlayerCount=currentCount
  
  if sel~="" then
   local playerStillExists=false
   for _,name in ipairs(currentPlayers)do
    if name==sel then playerStillExists=true break end
   end
...
Teleport:Button({
 ...
 Callback=function()
  if refreshPlayerList()then
   Teleport:Notify({Title="Player List Refreshed",Content="Player list updated.",Duration=2})
  else
   Teleport:Notify({Title="No Changes",Content="Player list unchanged.",Duration=2})
  end
 end
})
...
Teleport:Button({
 ...
 Callback=function()
  if sel~="" then
   ...
   if tc and lc then
    lc.CFrame=tc.CFrame
    Teleport:Notify({Title="Teleported",Content="Teleported to "..sel,Duration=3})
   else
    Teleport:Notify({Title="Failed",Content="Player not found or no character.",Duration=3})
   end
  else
   Teleport:Notify({Title="Failed",Content="Select a player first.",Duration=3})
  end
 end
})
...
local function GetDiamondQuestProgress()
local result = {
...
if (qData.CurrentObj or 0) > 6 then
result.AllDone = true
result.Q1.Done = true; result.Q2.Done = true; result.Q3.Done = true
result.Q4.Done = true; result.Q5.Done = true; result.Q6.Done = true
result.Q1.Text = "Element Rod: Done"; result.Q2.Text = "Coral Secret: Done"
result.Q3.Text = "Tropical Secret: Done"; result.Q4.Text = "Mutated Ruby: Done"
result.Q5.Text = "Lochness: Done"; result.Q6.Text = "Perfect Throws: Done"
return result
end
for id, objData in pairs(qData.Objectives) do
local prog = objData.Progress or 0
local numId = tonumber(id)
if numId == 1 then result.Q1.Text = string.format("Element Rod: %d/1", prog); result.Q1.Done = prog >= 1
elseif numId == 2 then result.Q2.Text = string.format("Coral Secret: %d/1", prog); result.Q2.Done = prog >= 1
elseif numId == 3 then result.Q3.Text = string.format("Tropical Secret: %d/1", prog); result.Q3.Done = prog >= 1
elseif numId == 4 then result.Q4.Text = string.format("Mutated Ruby: %d/1", prog); result.Q4.Done = prog >= 1
elseif numId == 5 then result.Q5.Text = string.format("Lochness: %d/1", prog); result.Q5.Done = prog >= 1
elseif numId == 6 then result.Q6.Text = string.format("Perfect Throws: %d/1000", prog); result.Q6.Done = prog >= 1000 end
end
else result.BoardFound = false end
return result
end
...
local hasSingularity = HasItemByID(18, "Bait")
if not hasSingularity and currentCoins >= 8200000 then pcall(function() RF_PurchaseBait:InvokeServer(18) end) WindUI:Notify({Title = "Upgrade Bait", Content = "Membeli Singularity Bait.", Duration = 3, Icon = "star"}) end
end
end
...
if qData and qData.Objectives then
if (qData.CurrentObj or 0) > 4 then
result.AllDone = true
result.Q1.Done = true; result.Q2.Done = true; result.Q3.Done = true; result.Q4.Done = true
result.Q1.Text = "Rare/Epic: Done"; result.Q2.Text = "Mythic: Done"; result.Q3.Text = "Secret: Done"; result.Q4.Text = "Coins: Done"
return result
end
for id, objData in pairs(qData.Objectives) do
local prog = objData.Progress or 0
...
local result = {
Q1 = {Text = "Own Ghostfin Rod", Done = false},
Q2 = {Text = "Secret (Jungle)", Done = false},
Q3 = {Text = "Secret (Temple)", Done = false},
...
result.AllDone = true
result.Q1.Done = true; result.Q2.Done = true; result.Q3.Done = true; result.Q4.Done = true
result.Q4.Text = "Stones: Done"
return result
end
for id, objData in pairs(qData.Objectives) do
local prog = objData.Progress or 0
local numId = tonumber(id)
if numId == 1 then result.Q1.Done = prog >= 1 end
if numId == 2 then result.Q2.Done = prog >= 1 end
if numId == 3 then result.Q3.Done = prog >= 1 end
if numId == 4 then result.Q4.Text = string.format("Stones: %d/3", prog); result.Q4.Done = prog >= 3 end
end
else result.BoardFound = false end
return result
end
...
makeLabel("", nil, 16, nil, 4)
makeLabel("Progress Quest Diamond Rod", Color3.fromRGB(255, 215, 0), 17, Enum.Font.GothamBold, 14)
local lDiamond1 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), 18, Enum.Font.Gotham, 11)
local lDiamond2 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), 19, Enum.Font.Gotham, 11)
local lDiamond3 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), 20, Enum.Font.Gotham, 11)
local lDiamond4 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), 21, Enum.Font.Gotham, 11)
local lDiamond5 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), 22, Enum.Font.Gotham, 11)
local lDiamond6 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), 23, Enum.Font.Gotham, 11)
makeLabel("----------------------------------------------------------------------------------------------------", Color3.fromRGB(100, 100, 100), 24, nil, 8)
makeLabel("CURRENT ACTIVITY", Color3.fromRGB(255, 215, 0), 25, Enum.Font.GothamBold, 13)
local lStatus = makeLabel("Idle", Color3.fromRGB(0, 255, 127), 26, Enum.Font.GothamBlack, 16)
...
local pGhost = GetGhostfinProgressSafe()
local pElem = GetElementProgressSafe()
local pDiamond = GetDiamondQuestProgress()
...
elseif step == 5 then
local hasAres = HasItemByID(126, "Fishing Rods")
if not hasAres and coins >= 3000000 then
currentActivityText = "Upgrading to ARES ROD..."
ForceResetAndTeleport(nil, nil)
pcall(function() RF_PurchaseFishingRod:InvokeServer(126) end)
task.wait(1.5)
EquipBestGear()
else
if not pGhost.BoardFound then
currentActivityText = "Reading Quest Board..."
TeleportToLookAt(SECOND_ALTAR_POS, SECOND_ALTAR_LOOK)
task.wait(2)
else
if pGhost.AllDone then
currentActivityText = "Ghostfin Quest Done! Equipping..."
task.wait(2)
EquipBestGear()
else
...
elseif step == 6 then
if not pElem.BoardFound then
currentActivityText = "Reading Element Quest Board..."
TeleportToLookAt(SECOND_ALTAR_POS, SECOND_ALTAR_LOOK)
task.wait(2)
else
if pElem.AllDone then
currentActivityText = "Element Quest Done! Equipping..."
task.wait(2)
EquipBestGear()
else
local hrp = game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
...
currentActivityText = "Complete Puzzle"
...
elseif step == 7 then
if HasDiamondRod() then
currentActivityText = "DIAMOND ROD OBTAINED! ALL DONE."
task.wait(3)
...
if not pDiamond.BoardFound then
currentActivityText = "Reading Diamond Quest Board..."
TeleportToLookAt(SECOND_ALTAR_POS, SECOND_ALTAR_LOOK)
task.wait(2)
else
...
currentActivityText = "Farming Secret Fish (Coral Reefs)"
...
currentActivityText = "Farming Secret Fish (Tropical Grove)"
...
RunQuestInstantFish(CURRENT_KAITUN_DELAY)
elseif not pDiamond.Q4.Done or not pDiamond.Q5.Done then
currentActivityText = "Farming Ruby/Lochness (Sisyphus)"
if (hrp.Position - SISYPHUS_POS).Magnitude > 15 then TeleportToLookAt(SISYPHUS_POS, SISYPHUS_LOOK) task.wait(0.5) end
RunQuestInstantFish(CURRENT_KAITUN_DELAY)
elseif not pDiamond.Q6.Done then
currentActivityText = string.format("Perfect Throws: %d/1000", pDiamond.Q6.Text:match("%d+") or 0)
if (hrp.Position - TROPICAL_GROVE_POS).Magnitude > 15 then TeleportToLookAt(TROPICAL_GROVE_POS, TROPICAL_GROVE_LOOK) task.wait(0.5) end
RunQuestInstantFish(CURRENT_KAITUN_DELAY)
end
end
...
elseif step == 8 then
currentActivityText = "KAITUN COMPLETED! DIAMOND ROD ACQUIRED."
task.wait(3)
if KAITUN_ACTIVE then
KAITUN_ACTIVE = false
WindUI:Notify({Title = "Kaitun Selesai", Content = "Diamond Rod Acquired!", Duration = 5, Icon = "check"})
...
local DeepSeaParagraph = deepsea:Paragraph({
...
-- Lokasi quest (TIDAK PERLU ALTAR karena quest one-time)
...
local function GetDeepSeaStatus()
    local result = {
        QuestCompleted = false,  -- Quest sudah selesai (one-time)
        QuestActive = false,     -- Quest sedang aktif
        Progress = nil,          -- Data progress jika sedang aktif
        HasGhostfinRod = false   -- Sudah punya Ghostfin Rod
    }
    
    -- 1. Cek apakah quest sudah selesai (di CompletedQuests)
    ...
    -- 3. Cek apakah quest sedang aktif (hanya jika belum selesai)
    if not result.QuestCompleted then
        local questData = ClientData:Get({"Quests", "Mainline", "Deep Sea Quest"})
        if questData then
            result.QuestActive = true
            result.Progress = {
                Q1 = {Progress = 0, Done = false}, -- Rare/Epic
                Q2 = {Progress = 0, Done = false}, -- Mythic
                Q3 = {Progress = 0, Done = false}, -- Secret
                Q4 = {Progress = 0, Done = false}, -- Coins
                TotalPercent = 0,
                AllDone = false
            }
        ...
        while runningDeepSea do
            ...
            if tick() - lastDisplayUpdate > 1 then
                if status.QuestCompleted then
                    -- QUEST SUDAH SELESAI (one-time)
                    DeepSeaParagraph:SetTitle("DEEP SEA COMPLETED")
                    if status.HasGhostfinRod then
                        DeepSeaParagraph:SetDesc("Quest sudah selesai!\nGhostfin Rod sudah didapat. ✅")
                    else
                        DeepSeaParagraph:SetDesc("Quest sudah selesai!\nAmbil Ghostfin Rod di altar.")
                    end
                ...
                elseif status.QuestActive then
                    ...
                    -- QUEST BELUM PERNAH DIMULAI
                    DeepSeaParagraph:SetTitle("DEEP SEA NOT STARTED")
                    DeepSeaParagraph:SetDesc("Quest belum pernah dimulai.\nPergi ke Deep Sea area untuk memulai.")
                end
                lastDisplayUpdate = tick()
            end
            
            -- LOGIKA UTAMA: Handle semua status
            if status.QuestCompleted then
                -- QUEST SUDAH SELESAI: auto stop
                DeepSeaParagraph:SetTitle("QUEST COMPLETED")
                DeepSeaParagraph:SetDesc("Deep Sea Quest sudah selesai!\nToggle dimatikan otomatis.")
                runningDeepSea = false
                if DeepSeaToggle then
                    DeepSeaToggle:Set(false)
                end
                break
                
            elseif not status.QuestActive then
                -- QUEST BELUM DIMULAI: tidak ada teleport ke altar
            ...
            elseif status.QuestActive and status.Progress.AllDone then
                -- QUEST AKTIF & SUDAH SELESAI semua objective
                DeepSeaParagraph:SetTitle("QUEST OBJECTIVES DONE!")
                DeepSeaParagraph:SetDesc("Semua objective selesai!\nKembali ke altar untuk claim reward.")
                runningDeepSea = false
                if DeepSeaToggle then
                    DeepSeaToggle:Set(false)
                end
            ...
            -- FARMING LOGIC (hanya jika quest aktif & belum selesai)
            if status.QuestActive then
                local activity = ""
                local targetCF = nil
                
                if not status.Progress.Q1.Done then
                    activity = "Farming Rare/Epic di Treasure Room"
                    targetCF = TREASURE_ROOM_CF
                    
                elseif not status.Progress.Q2.Done or not status.Progress.Q3.Done then
                    activity = "Farming Mythic/Secret di Sisyphus"
                    targetCF = SISYPHUS_CF
                    
                elseif not status.Progress.Q4.Done then
                    activity = string.format("Collecting Coins: %s/1M", tostring(status.Progress.Q4.Progress))
                    targetCF = SISYPHUS_CF
                end
    ...
    Callback = function(s)
        ...
        if s then
            ...
            if status.QuestCompleted then
                ...
                DeepSeaParagraph:SetDesc("Deep Sea Quest sudah selesai sebelumnya.\nToggle dimatikan otomatis.")
                runningDeepSea = false
                if DeepSeaToggle then
                    DeepSeaToggle:Set(false)
                end
            ...
            elseif not status.QuestActive then
                DeepSeaParagraph:SetTitle("⚠️ QUEST NOT STARTED")
                DeepSeaParagraph:SetDesc("Pergi ke area Deep Sea untuk memulai quest.")
...
local ElementParagraph = element:Paragraph({
...
local QuestModules = require(ReplicatedStorage.Modules.Quests)
local ElementQuestData = QuestModules.Mainline["Element Quest"]
...
local function GetElementQuestStatus()
    ...
    if not result.QuestCompleted then
        ...
        if questData then
            result.QuestActive = true
            result.Progress = {
                Q1 = {Done = true, Text = "Ghostfin Rod: ✅"}, 
                Q2 = {Progress = 0, Done = false, Text = "Jungle Secret: ❌"},
                Q3 = {Progress = 0, Done = false, Text = "Temple Secret: ❌"},
                Q4 = {Progress = 0, Done = false, Text = "Transcended Stones: 0/3"},
                TotalPercent = 0,
                AllDone = false
            }
            
            -- Ambil progress objective Element Quest
            for objId, objData in pairs(questData.Objectives) do
                local numId = tonumber(objId)
                local progress = objData.Progress or 0
                
                if numId == 2 then -- Jungle Secret
                    result.Progress.Q2.Progress = progress
                    result.Progress.Q2.Done = progress >= 1
                    result.Progress.Q2.Text = result.Progress.Q2.Done and "Jungle Secret: ✅" or "Jungle Secret: ❌"
                    
                elseif numId == 3 then -- Temple Secret
                    result.Progress.Q3.Progress = progress
                    result.Progress.Q3.Done = progress >= 1
                    result.Progress.Q3.Text = result.Progress.Q3.Done and "Temple Secret: ✅" or "Temple Secret: ❌"
                    
                elseif numId == 4 then -- Transcended Stones
                    result.Progress.Q4.Progress = progress
            ...
            -- Objective 1 (Ghostfin Rod) otomatis true jika bisa start
        ...
        while runningElement do
            ...
            if tick() - lastDisplayUpdate > 1 then
                if status.QuestCompleted then
                    ...
                    ElementParagraph:SetDesc("Quest sudah selesai!\nElement Rod sudah didapat.")
                ...
                elseif status.QuestActive then
                    ...
                    -- BISA MULAI TAPI BELUM AKTIF
                    ElementParagraph:SetTitle("⚠️ READY TO START")
                    if status.DeepSeaCompleted then
                        ElementParagraph:SetDesc("Deep Sea Quest selesai ✅\nPergi ke altar untuk mulai Element Quest")
                    elseif status.HasGhostfinRod then
                        ElementParagraph:SetDesc("Sudah punya Ghostfin Rod ✅\nPergi ke altar untuk mulai Element Quest")
                    end
                end
                lastDisplayUpdate = tick()
            end
            ...
            if status.QuestCompleted then
                ...
                ElementParagraph:SetDesc("Element Quest sudah selesai!\nToggle dimatikan otomatis.")
                runningElement = false
                if ElementToggle then
                    ElementToggle:Set(false)
                end
                break
                
            elseif not status.CanStartElement then
                -- BELUM BISA MULAI: beri info jelas
                ElementParagraph:SetTitle("❌ CAN'T START ELEMENT")
                ElementParagraph:SetDesc("Selesaikan Deep Sea Quest dulu\natau dapatkan Ghostfin Rod")
                runningElement = false
                if ElementToggle then
                    ElementToggle:Set(false)
                end
            ...
            elseif not status.QuestActive then
                -- BISA MULAI TAPI BELUM AKTIF: teleport ke altar
            ...
            elseif status.QuestActive and status.Progress.AllDone then
                ...
                ElementParagraph:SetDesc("Semua objective selesai!\nKembali ke altar untuk claim Element Rod.")
                runningElement = false
                if ElementToggle then
                    ElementToggle:Set(false)
                end
            ...
            if status.QuestActive then
                ...
                if not status.Progress.Q2.Done then
                    activity = "Farming Secret Fish di Jungle"
                    targetCF = JUNGLE_CF
                    
                elseif not status.Progress.Q3.Done then
                    activity = "Farming Secret Fish di Temple"
                    targetCF = TEMPLE_CF
                    
                elseif not status.Progress.Q4.Done then
                    activity = string.format("Membuat Transcended Stones: %d/3", status.Progress.Q4.Progress)
                    targetCF = ALTAR_CF
                end
    ...
    Callback = function(s)
        ...
        if s then
            ...
            if status.QuestCompleted then
                ...
                ElementParagraph:SetDesc("Element Quest sudah selesai sebelumnya.\nToggle dimatikan otomatis.")
                runningElement = false
                if ElementToggle then
                    ElementToggle:Set(false)
                end
            ...
            elseif not status.CanStartElement then
                ElementParagraph:SetTitle("❌ CAN'T START ELEMENT QUEST")
                ElementParagraph:SetDesc(string.format(
                    "Butuh:\n- Deep Sea Quest selesai: %s\n- Punya Ghostfin Rod: %s",
                    status.DeepSeaCompleted and "✅" or "❌",
                    status.HasGhostfinRod and "✅" or "❌"
                ))
                runningElement = false
                if ElementToggle then
                    ElementToggle:Set(false)
...
local DiamondParagraph = deepsea:Paragraph({
    Title = "Diamond Quest Monitor",
    Content = "Initializing...",
    Icon = "gem"
})
...
-- Lokasi quest
local CORAL_REEFS_CF = CFrame.new(-3020, 3, 2260) -- Coral Reefs
local TROPICAL_GROVE_CF = CFrame.new(-2150, 53, 3672) -- Tropical Grove
local RUBY_FARM_CF = CFrame.new(-3595, -279, -1589) -- Farm Ruby (Treasure Room)
local LOCHNESS_FARM_CF = CFrame.new(-712, 6, 707) -- Farm Lochness Monster
...
-- Remote untuk claim Diamond Rod
...
-- Function untuk claim Diamond Rod
local function ClaimDiamondRod()
    local claimRemote = GetClaimRemote()
    if claimRemote then
        local success, result = pcall(function()
            return claimRemote:InvokeServer("Diamond Rod")
        end)
        if success then
            -- Cek apakah benar dapat Diamond Rod
            task.wait(1) -- Tunggu data update
...
local function GetDiamondQuestStatus()
    local result = {
        QuestCompleted = false,      -- Diamond Quest sudah selesai
        QuestActive = false,         -- Diamond Quest sedang aktif
        HasElementRod = false,       -- Sudah punya Element Rod
        HasDiamondRod = false,      
        HasDiamondKey = false,       
        Progress = nil,              
        InventoryCheck = {           
        ...
        CanClaimDiamondRod = false   
    }
    
    local completedQuests = ClientData:Get({"CompletedQuests"}) or {}
    for _, questName in ipairs(completedQuests) do
        if questName == "Diamond Rod Quest" then
            result.QuestCompleted = true
        end
    end
    ...
    if inventory then
        if inventory["Fishing Rods"] then
            for _, rod in ipairs(inventory["Fishing Rods"]) do
                if tonumber(rod.Id) == ELEMENT_ROD_ID then
                    result.HasElementRod = true
                end
                if tonumber(rod.Id) == DIAMOND_ROD_ID then
                    result.HasDiamondRod = true
                end
            end
        end
        ...
        if inventory.Items then
            for _, item in ipairs(inventory.Items) do
                ...
                if itemData and itemData.Data then
                    if itemData.Data.Name == "Diamond Key" then
                        result.HasDiamondKey = true
                    end
                end
                ...
                if tonumber(item.Id) == RUBY_ID then
                    ...
                    if metadata.VariantId == 3 then
                        result.InventoryCheck.HasRuby = true
                    end
    ...
    result.CanClaimDiamondRod = result.QuestCompleted and result.HasDiamondKey and not result.HasDiamondRod
    ...
    if not result.QuestCompleted then
        ...
        if questData then
            result.QuestActive = true
            result.Progress = {
                Q1 = {Done = result.HasElementRod, Text = result.HasElementRod and "Element Rod: ✅" or "Element Rod: ❌"},
                Q2 = {Progress = 0, Done = false, Text = "Coral Secret: ❌"},
                Q3 = {Progress = 0, Done = false, Text = "Tropical Secret: ❌"},
                Q4 = {Done = false, Text = "Mutated Ruby: ❌"},
                Q5 = {Done = false, Text = "Lochness Monster: ❌"},
                Q6 = {Progress = 0, Done = false, Text = "Perfect Throws: 0/1000"},
                TotalPercent = 0,
                AllDone = false
            }
            
            -- Ambil progress dari quest data
            for objId, objData in pairs(questData.Objectives) do
                local numId = tonumber(objId)
                local progress = objData.Progress or 0
                
                if numId == 1 then -- Element Rod
                    result.Progress.Q1.Done = progress >= 1 or result.HasElementRod
                    result.Progress.Q1.Text = result.Progress.Q1.Done and "Element Rod: ✅" or "Element Rod: ❌"
                    
                elseif numId == 2 then -- Coral Reefs Secret
                    result.Progress.Q2.Progress = progress
                    result.Progress.Q2.Done = progress >= 1
                    result.Progress.Q2.Text = result.Progress.Q2.Done and "Coral Secret: ✅" or string.format("Coral Secret: %d/1", progress)
                    
                elseif numId == 3 then -- Tropical Grove Secret
                    result.Progress.Q3.Progress = progress
                    result.Progress.Q3.Done = progress >= 1
                    result.Progress.Q3.Text = result.Progress.Q3.Done and "Tropical Secret: ✅" or string.format("Tropical Secret: %d/1", progress)
                    
                elseif numId == 4 then -- Mutated Ruby
                    result.Progress.Q4.Done = progress >= 1 or result.InventoryCheck.HasRuby
                    result.Progress.Q4.Text = result.Progress.Q4.Done and "Mutated Ruby: ✅" or "Mutated Ruby: ❌"
                    
                elseif numId == 5 then -- Lochness Monster
                    result.Progress.Q5.Done = progress >= 1 or result.InventoryCheck.HasLochness
                    result.Progress.Q5.Text = result.Progress.Q5.Done and "Lochness Monster: ✅" or "Lochness Monster: ❌"
...
local function RunDiamondQuest()
    ...
    diamondThread = task.spawn(function()
        local lastDisplayUpdate = 0
        local claimedRod = false -- Flag untuk mencegah spam claim
        ...
        while runningDiamond do
            ...
            if tick() - lastDisplayUpdate > 1 then
                if status.HasDiamondRod then
                    ...
                    DiamondParagraph:SetDesc("Diamond Rod (ID: 559) sudah didapat!\nQuest fully completed.")
                    
                elseif status.CanClaimDiamondRod and not claimedRod then
                    DiamondParagraph:SetTitle("🎉 CLAIM DIAMOND ROD")
                    DiamondParagraph:SetDesc("Quest selesai & punya Diamond Key!\nClick toggle to claim Diamond Rod...")
                    
                elseif status.QuestCompleted and not status.HasDiamondKey then
                    DiamondParagraph:SetTitle("⚠️ GET DIAMOND KEY")
                    DiamondParagraph:SetDesc("Quest selesai tapi belum dapat Diamond Key.\nKembali ke Lary untuk claim reward.")
                    
                elseif not status.QuestActive then
                    if not status.HasElementRod then
                        DiamondParagraph:SetTitle("❌ NEED ELEMENT ROD")
                        DiamondParagraph:SetDesc("Butuh Element Rod (ID: 257) untuk memulai quest.")
                    else
                        DiamondParagraph:SetTitle("⚠️ READY TO START")
                        DiamondParagraph:SetDesc("Sudah punya Element Rod ✅\nPergi ke Lary untuk mulai quest")
                    end
            ...
            if status.HasDiamondRod then
                DiamondParagraph:SetTitle("✅ DIAMOND ROD OBTAINED")
                DiamondParagraph:SetDesc("Diamond Rod sudah didapat!\nToggle dimatikan otomatis.")
                runningDiamond = false
                if DiamondToggle then
                    DiamondToggle:Set(false)
                end
                break
                
            elseif status.CanClaimDiamondRod and not claimedRod then
                -- AUTO CLAIM DIAMOND ROD
                ...
                if success then
                    ...
                    claimedRod = true
                    task.wait(2) -- Tunggu data update
                else
            ...
            elseif not status.HasElementRod then
                ...
                DiamondParagraph:SetDesc("Selesaikan Element Quest dulu!\nToggle dimatikan otomatis.")
                runningDiamond = false
                if DiamondToggle then
                    DiamondToggle:Set(false)
                end
            ...
            elseif not status.QuestActive then
                DiamondParagraph:SetTitle("⚠️ GO TO LARY")
                DiamondParagraph:SetDesc("Pergi ke Lary the Scientist untuk memulai quest.")
                task.wait(3)
                continue
                
            elseif status.QuestActive and status.Progress.AllDone then
                DiamondParagraph:SetTitle("🎉 QUEST OBJECTIVES DONE!")
                DiamondParagraph:SetDesc("Semua objective selesai!\nKembali ke Lary untuk claim Diamond Key.")
                runningDiamond = false
                if DiamondToggle then
                    DiamondToggle:Set(false)
                end
            ...
            if status.QuestActive then
                ...
                -- Priority order untuk farming
                if not status.Progress.Q4.Done and not status.InventoryCheck.HasRuby then
                    activity = "Farming Mutated Ruby di Treasure Room"
                    targetCF = RUBY_FARM_CF
                    
                elseif not status.Progress.Q5.Done and not status.InventoryCheck.HasLochness then
                    activity = "Farming Lochness Monster"
                    targetCF = LOCHNESS_FARM_CF
                    
                elseif not status.Progress.Q2.Done then
                    activity = "Farming Secret Fish di Coral Reefs"
                    targetCF = CORAL_REEFS_CF
                    
                elseif not status.Progress.Q3.Done then
                    activity = "Farming Secret Fish di Tropical Grove"
                    targetCF = TROPICAL_GROVE_CF
                    
                elseif not status.Progress.Q6.Done then
                    activity = string.format("Perfect Throws: %d/1000", status.Progress.Q6.Progress)
                    targetCF = TROPICAL_GROVE_CF
                end
...
local DiamondToggle = Reg("autoDiamond", deepsea:Toggle({
    Title = "Auto Complete Diamond Quest",
    Desc = "Farm → Claim → Get Diamond Rod (ID: 559)",
    Value = false,
    Callback = function(s)
        runningDiamond = s
        if s then
            ...
            if status.HasDiamondRod then
                ...
                DiamondParagraph:SetDesc("Sudah punya Diamond Rod (ID: 559).\nToggle dimatikan otomatis.")
                runningDiamond = false
                if DiamondToggle then
                    DiamondToggle:Set(false)
                end
            ...
            elseif status.CanClaimDiamondRod then
                DiamondParagraph:SetTitle("🎉 READY TO CLAIM")
                DiamondParagraph:SetDesc("Quest selesai & punya Diamond Key!\nScript akan auto claim Diamond Rod.")
                
            elseif not status.HasElementRod then
                DiamondParagraph:SetTitle("❌ NEED ELEMENT ROD FIRST")
                DiamondParagraph:SetDesc("Butuh Element Rod (ID: 257) dulu!\nToggle dimatikan otomatis.")
                runningDiamond = false
                if DiamondToggle then
                    DiamondToggle:Set(false)
                end
            ...
            elseif status.QuestActive and status.Progress.AllDone then
                DiamondParagraph:SetTitle("🎉 READY TO CLAIM KEY")
                DiamondParagraph:SetDesc("Semua objective selesai!\nKembali ke Lary untuk claim Diamond Key.")
                runningDiamond = false
                if DiamondToggle then
                    DiamondToggle:Set(false)
...
if pr and pn then table.insert(out,{UUID=item.UUID,Name=name,Id=item.Id,Metadata=item.Metadata or{}})end
end
return out
end
...
local tradeDropdown
Trade:Input({Title="Search Item Name",Placeholder="e.g. Aurora",Icon="search",Callback=function(t)
if not tradeDropdown then return end
if t==""or#t<2 then tradeDropdown:Refresh({"Ketik minimal 2 huruf..."})return end
local res,cnt={},0
local l=t:lower()
for _,n in ipairs(GlobalItemCache)do
if n:lower():find(l)then table.insert(res,n)cnt+=1 if cnt>=25 then break end end
end
if #res==0 then table.insert(res,"Not Found")end
tradeDropdown:Refresh(res)
end})
tradeDropdown=Trade:Dropdown({Title="Select Item to Trade",Desc="Pilih item spesifik dari hasil pencarian.",Values={"Ketik nama item di atas..."},Multi=false,AllowNone=true,Callback=function(n)selectedTradeItemName=n end})
...
local look=Vector3.new(data.lx or 0,data.ly or 0,data.lz or -1)
