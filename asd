...
local run=false
local loopT,eqT,attrT=nil,nil,nil

local cDelay=3.1
local kDelay=.3
local interval=1.8

local loopIntervalInput=Reg("loopInterval",Auto:Input({
    Title="Recast Delay",
...
local completeDelayInput=Reg("completeDelay",Auto:Input({
    Title="Complete Delay",
    Value=tostring(cDelay),
    Placeholder="3.1",
    Callback=function(v)
        local n=tonumber(v)
...
local UpdateAutoFishing=Net:RemoteFunction("UpdateAutoFishingState")
...
local function cast()
    ...
    task.spawn(function()
        ...
        pcall(function()
            RFS:InvokeServer(-139.6379699707,0.99647927980797)
        end)
...
local function startAttributeGuard()
    pcall(function()
        LocalPlayer:SetAttribute("InCutscene", true)
    end)
    attrT = nil  -- Gak perlu loop
end

local function stopAttributeGuard()
    if attrT then
        task.cancel(attrT)
        attrT=nil
    end
