Fluent, SaveManager, InterfaceManager = loadstring(Game:HttpGet("https://raw.githubusercontent.com/discoart/FluentPlus/refs/heads/main/Beta.lua"))()
local Window=Fluent:CreateWindow({Title="Fyy X Fish IT | 1.1.2",SubTitle="by Fyy Community",TabWidth=150,Size=UDim2.fromOffset(530, 300),Acrylic=true,Theme="Darker",MinimizeKey=Enum.KeyCode.G,BackgroundImage="rbxassetid://78893380921225",BackgroundTransparency=0.4})
local Tabs={Auth=Window:AddTab({Title="Authentication",Icon="key"})}
local Options=Fluent.Options
-- Ditunda setup SaveManager hingga setelah authentication berhasil
-- Ini mencegah autoload config sebelum tabs dan authentication selesai
        ...
        if s then
            dorianThread = task.spawn(function()
                ...
                if dorianRunning then
                    Fluent:Notify({Title="Dorian Quest", Content="Collecting 33 Coral...", Duration=3})
                    for i = 1, 33 do
...
local function EventTab()
    ...
    pirateRewardToggle:OnChanged(function(s)
        ...
        if s then
            ...
            Fluent:Notify({Title="Auto Claim ON", Duration=3})
            ...
            Fluent:Notify({Title="Auto Claim OFF", Duration=2})
        end
    end)
    ...
    treasureToggle:OnChanged(function(state)
        ...
        if state then
            ...
            Fluent:Notify({Title="Auto Claim ON", Duration=2})
            triedChests = {}
...
local function TradeTab()
    ...
    local function RunAutoTradeLoop()
        ...
        autoTradeThread = task.spawn(function()
            ...
            Fluent:Notify({Title="Auto Trade Stopped", Duration=3})
        end)
    end
    ...
    Trade:AddToggle("EnableAutoTrade", {
        ...
        if s then
            ...
            if autoTradeThread then task.cancel(autoTradeThread) end
            Fluent:Notify({Title="Stopped", Duration=2})
        end
    end)
end
...
local function EnchantTab()
    ...
    local function RunAutoEnchantLoop(uuid, stoneType)
        ...
        autoEnchantThread = task.spawn(function()
            ...
            Fluent:Notify({Title="Auto Enchant Stopped", Duration=3})
        end)
    end
