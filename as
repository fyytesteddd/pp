...
local instantv1=Auto:Section({Title="Instant Fishing",TextSize=16})
local AFR=false
local RS=game:GetService("ReplicatedStorage")
...
local autoFishingToggle = Reg("autoFishing", instantv1:Toggle({Title="Instant Fishing",Default=false,Callback=function(s) run=s AFR=s
 if run then task.spawn(start)
 else sI(RFK)stop()eq=false end
end}))

local fishingDelaySlider = Reg("fishingDelay", instantv1:Slider({Title="Completed Delay",Step=.1,Value={Min=.1,Max=5,Default=1.3},Callback=function(v)d=v end}))
...
local blatantv1=Auto:Section({Title="Blatant V1",TextSize=16})
...
local loopIntervalInput=Reg("loopInterval",blatantv1:Input({Title="Recast Delay",Value=tostring(interval),Placeholder="1.8",Callback=function(v)local n=tonumber(v)if n and n>=0 then interval=n end end}))
local completeDelayInput=Reg("completeDelay",blatantv1:Input({Title="Complete Delay",Value=tostring(cDelay),Placeholder="3.1",Callback=function(v)local n=tonumber(v)if n and n>0 then cDelay=n end end}))
...
local blatantModeToggle = Reg("blatantMode",blatantv1:Toggle({
...
local blatantv21=Auto:Section({Title="Blatant V2",TextSize=16})
local blatantV2CompleteDelayInput=Reg("blatantV2_completeDelay",blatantv21:Input({Title="Complete Delay",Placeholder=tostring(FD),Value=tostring(FD),Callback=function(v)local n=tonumber(v)if n and n>=0 then FD=n end end}))
local blatantV2CancelDelayInput=Reg("blatantV2_cancelDelay",blatantv21:Input({Title="Cancel Delay",Placeholder=tostring(KD),Value=tostring(KD),Callback=function(v)local n=tonumber(v)if n and n>=0 then KD=n end end}))

local blatantV2Toggle=Reg("blatantV2_toggle",blatantv21:Toggle({
...
local blatantv3=Auto:Section({Title="Blatant V3 (BETA)",TextSize=16})
local blatantV3CastDelayInput=Reg("blatantV3_castDelay",blatantv3:Input({Title="Cast Delay",Placeholder=tostring(CD),Value=tostring(CD),Callback=function(v)local n=tonumber(v)if n and n>=0 then CD=n end end}))
local blatantV3CompleteDelayInput=Reg("blatantV3_completeDelay",blatantv3:Input({Title="Complete Delay",Placeholder=tostring(FD),Value=tostring(FD),Callback=function(v)local n=tonumber(v)if n and n>=0 then FD=n end end}))
local blatantV3CancelDelayInput=Reg("blatantV3_cancelDelay",blatantv3:Input({Title="Cancel Delay",Placeholder=tostring(KD),Value=tostring(KD),Callback=function(v)local n=tonumber(v)if n and n>=0 then KD=n end end}))

local blatantV3Toggle=Reg("blatantV3_toggle",blatantv3:Toggle({
Title="Enable Blatant V3",
Desc="Ultra fast perfect",
Default=false,
Callback=function(s)
active=s
if s then 
...
--// ASD
local Teleport1=Auto:Section({Title="Teleport Feature",TextSize=16})
...
local teleportLocationDropdown = Reg("teleportLocation",Teleport1:Dropdown({
...
local autoTeleportToggle = Reg("autoTeleport",Teleport1:Toggle({
...
Teleport1:Button({Title="Save Your Position",Callback=save})

local TT=Teleport1:Toggle({
 Title="Teleport & Freeze to Position",
 Default=false,
 Callback=function(s)
  on=s
...
local Autosell=Auto:Section({Title="Auto Sell Feature",TextSize=16})
...
local sellValueInput = Reg("sellValue", Autosell:Input({
 Title="Value",
 Placeholder="50",
 Value="50",
 Icon="hash",
...
local autoSellToggle = Reg("autoSell", Autosell:Toggle({
...
Autosell:Button({Title="Sell Now",Callback=function()sell()end})
Auto:Divider()
--// Auto Sell Feature
local RS=game:GetService("ReplicatedStorage")
...
local allM={"Shiny","Albino","Sandy","Noob","Moon Fragment","Festive","Disco","1x1x1x1","Bloodmoon","Color Burn","Corrupt","Fairy Dust","Frozen","Galaxy","Gemstone","Ghost","Gold","Holographic","Lightning","Midnight","Radioactive","Stone","Leviathan's Rage"}
local rar={"Common","Uncommon","Rare","Epic","Legendary","Mythic","SECRET"}

local favorite1=Auto:Section({Title="Auto Favorite Feature",TextSize=16})

local favRarityDropdown=favorite1:Dropdown({Title="Rarity",Values=rar,Value={"Legendary","Mythic","SECRET"},Multi=true,Callback=function(v)tiers=v end})
local favMutationDropdown=favorite1:Dropdown({Title="Mutation",Values=allM,Value={},Multi=true,Callback=function(v)muts=v end})
...
local selectedItems={}
local favoriteByNameDropdown=favorite1:Dropdown({Title="Favorite by Name",Values=allItemNames,Multi=true,AllowNone=true,SearchBarEnabled=true,ScrollBarEnabled=true,Callback=function(v)selectedItems=v end})

local autoFavoriteToggle=favorite1:Toggle({Title="Enable Auto Favorite",Default=false,Callback=function(s)
 if s then
...
makeLabel("Fyy Kaitun Status", Color3.fromRGB(255, 255, 255), UDim2.new(0, 10, 0, 10), Enum.Font.GothamBlack, 18)
local lStats1 = makeLabel("Loading...", Color3.fromRGB(255, 255, 255), UDim2.new(0, 10, 0, 35), Enum.Font.GothamSemibold, 11)
local lStats2 = makeLabel("Loading...", Color3.fromRGB(255, 255, 255), UDim2.new(0, 10, 0, 55), Enum.Font.GothamSemibold, 11)
makeLabel("Ghostfin Quest", Color3.fromRGB(255, 100, 100), UDim2.new(0, 10, 0, 85), Enum.Font.GothamBold, 14)
local lGhost1 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0, 10, 0, 110), Enum.Font.Gotham, 11)
local lGhost2 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0, 10, 0, 130), Enum.Font.Gotham, 11)
local lGhost3 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0, 10, 0, 150), Enum.Font.Gotham, 11)
local lGhost4 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0, 10, 0, 170), Enum.Font.Gotham, 11)
makeLabel("Element Quest", Color3.fromRGB(100, 100, 255), UDim2.new(0.5, 10, 0, 85), Enum.Font.GothamBold, 14)
local lElem1 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.5, 10, 0, 110), Enum.Font.Gotham, 11)
local lElem2 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.5, 10, 0, 130), Enum.Font.Gotham, 11)
local lElem3 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.5, 10, 0, 150), Enum.Font.Gotham, 11)
local lElem4 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.5, 10, 0, 170), Enum.Font.Gotham, 11)
makeLabel("Diamond Quest", Color3.fromRGB(255, 215, 0), UDim2.new(0.25, -150, 0, 200), Enum.Font.GothamBold, 14)
local lDiamond1 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.25, -150, 0, 225), Enum.Font.Gotham, 11)
local lDiamond2 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.25, -150, 0, 245), Enum.Font.Gotham, 11)
local lDiamond3 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.25, -150, 0, 265), Enum.Font.Gotham, 11)
local lDiamond4 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.25, -150, 0, 285), Enum.Font.Gotham, 11)
local lDiamond5 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.25, -150, 0, 305), Enum.Font.Gotham, 11)
local lDiamond6 = makeLabel("Loading...", Color3.fromRGB(180, 180, 180), UDim2.new(0.25, -150, 0, 325), Enum.Font.Gotham, 11)
makeLabel("----------------------------------------------------------------------------------------------------", Color3.fromRGB(100, 100, 100), UDim2.new(0, 10, 0, 355), nil, 8)
makeLabel("CURRENT ACTIVITY", Color3.fromRGB(255, 215, 0), UDim2.new(0, 10, 0, 375), Enum.Font.GothamBold, 13)
local lStatus = makeLabel("Idle", Color3.fromRGB(0, 255, 127), UDim2.new(0, 10, 0, 400), Enum.Font.GothamBlack, 16)
...
Title="Auto Spawn 9 Totem",Value=false,Locked=true,
Callback=function(s)
...
local sectionclassic=Event:Section({Title="Pirate Event Rewards",TextSize=16})
sectionclassic:Toggle({Title="Auto Claim Pirate Event Rewards",Value=false,Icon="gift",Callback=function(s)
...
WindUI:Notify({Title="Config Loaded",Content="Config '"..ConfigName.."' loaded",Duration=3,Icon="folder-open"})
end})
...
WindUI:Notify({Title="Session Restored",Content="Teleported to last saved position.",Duration=4,Icon="map-pin"})
end
...
Callback=function(v)
AutoSavePosState=v
WindUI:Notify({Title=v and"Auto Save ON"or"Auto Save OFF",Duration=3})
end})
...
Callback=function(v)
AutoLoadConfig=v
WindUI:Notify({Title=v and"Auto Load ON"or"Auto Load OFF",Duration=3})
end})
...
pcall(SavePositionToFile)
IsLoadingPosition=t
WindUI:Notify({Title="Saved!",Content="Lokasi saat ini disimpan.",Duration=2,Icon="check"})
end})
...
SmartLoadConfig(ConfigName)
WindUI:Notify({Title="Auto Load",Content="Config berhasil di-load otomatis",Duration=3,Icon="check"})
end
end)
end
